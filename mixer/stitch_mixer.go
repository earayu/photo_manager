...
func (m *StitchMixer) Mix(imagePool []*image.Image) (*image.Image, error) {
	...
	//shuffle the images
	imagePool = common.ShuffleN(imagePool, m.PhotoCountInRow*m.PhotoCountInColumn)
	...
	outImg := image.NewRGBA(image.Rect(0, 0, w*m.PhotoCountInRow, h*m.PhotoCountInColumn))

	for i := 0; i < m.PhotoCountInRow; i++ {
		for j := 0; j < m.PhotoCountInColumn; j++ {
